...
- Tests must **fail** when server authority is violated in test doubles (client test must not assert paid balance without a mocked server response)
...
FiveM globals are not available in busted. Mock them at the top of each test file:
...
_G.source = 1  -- simulate server source
...
_G.QBCore = {
    Functions = {
        GetPlayer = function(src)
            ...
            return nil  -- simulate invalid/disconnected player
        end,
...
// Usage in test
const bridge = mockNuiBridge();
bridge.register('purchase', (data) => {
    if ((data as any).amount <= 0) return { ok: false, error: 'invalid_amount' };
    return { ok: true };
});
```
...
- name: Run NUI unit tests
  ...
  run: |
    npm ci
    npm run test -- --run   # Vitest CI mode (no watch)
```

- Use `--run` flag in Vitest for CI (exits after completion instead of watching)
- Pin busted version in `.busted` config to avoid CI drift
- Separate test script from build script in `package.json` (`"test": "vitest run"`, `"test:watch": "vitest"`)
...
- Do **not** implement production feature code unless tiny hooks are needed for testability (e.g., exporting a pure function)
